...
# **** MEASURE LOOP ****
async def measure():
    global tPRaw, tP, tPF, tRef, last_record_time, pi, handle, letFirstTempRecording, last_print_time
    while True:
...
#        print(tPF[0], tPF[1], tPF[2], tPF[3], tPF[4], tPF[5], tPF[6], tPF[7], tPF[8], tPF[9], tPF[10], tPF[11], tPF[12], tPF[13], tPF[14], tPF[15])
        ...
        if current_time - last_print_time >= 5:
            ...
            print('                                             R - MIN:', "{:.1f}".format(tRef * 0.1), f'REC:', "{:.1f}".format(tRecorded * 0.1), 'MAX:', "{:.1f}".format(tMax * 0.1), 'DIF:', "{:.1f}".format((tMax - tRecorded) * 0.1))
...
#        print(f'Average temperature: {tAverage:.2f}')
        ...
        #values_over_threshold = [value for value in tS if value > (tRecorded) + (config.threshold *10)]
